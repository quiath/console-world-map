...
function globe(t,w,nw,     v) {
     ...
     while(j<=slen){ 
      ...
      if (currc>0) {
        for(x=px;x<px+len;++x){
          ...
          if(dmax>dmin){
            v=1;
            if(currc in data)v=2+int(data[currc]/(dmax-dmin)*(npal-2)+0.5);
            wm[wy" "wx]="\033["pal[v]"m:";
          }
          else wm[wy" "wx]=(currc==C)?"\033[91m:\033[92m":":";
  ...
  for(i=1;i<=npal;++i)e=e"\033["pal[i]"m@";
  printf("%s", e);
...
BEGIN {
  ...
  nz=split("AFGAfghanistan AGOAngola ALBAlbania AREUnitedArabEmirates ARGArgentina ARMArmenia ATAAntarctica ATFFrenchSouthernandAntarctic AUSAustralia AUTAustria AZEAzerbaijan BDIBurundi BELBelgium BENBenin BFABurkinaFaso BGDBangladesh BGRBulgaria BHSTheBahamas BIHBosniaandHerzegovina BLRBelarus BLZBelize BOLBolivia BRABrazil BRNBrunei BTNBhutan BWABotswana CAFCentralAfricanRepublic CANCanada CHESwitzerland CHLChile CHNChina CIVIvoryCoast CMRCameroon CODDemocraticRepublicofthe COGRepublicoftheCongo COLColombia CRICostaRica CUBCuba XXNNorthernCyprus CYPCyprus CZECzechRepublic DEUGermany DJIDjibouti DNKDenmark DOMDominicanRepublic DZAAlgeria ECUEcuador EGYEgypt ERIEritrea ESPSpain ESTEstonia ETHEthiopia FINFinland FJIFiji FLKFalklandIslands FRAFrance GABGabon GBRUnitedKingdom GEOGeorgia GHAGhana GINGuinea GMBGambia GNBGuineaBissau GNQEquatorialGuinea GRCGreece GRLGreenland GTMGuatemala GUYGuyana HNDHonduras HRVCroatia HTIHaiti HUNHungary IDNIndonesia INDIndia IRLIreland IRNIran IRQIraq ISLIceland ISRIsrael ITAItaly JAMJamaica JORJordan JPNJapan KAZKazakhstan KENKenya KGZKyrgyzstan KHMCambodia KORSouthKorea XXKKosovo KWTKuwait LAOLaos LBNLebanon LBRLiberia LBYLibya LKASriLanka LSOLesotho LTULithuania LUXLuxembourg LVALatvia MARMorocco MDAMoldova MDGMadagascar MEXMexico MKDMacedonia MLIMali MMRMyanmar MNEMontenegro MNGMongolia MOZMozambique MRTMauritania MWIMalawi MYSMalaysia NAMNamibia NCLNewCaledonia NERNiger NGANigeria NICNicaragua NLDNetherlands NORNorway NPLNepal NZLNewZealand OMNOman PAKPakistan PANPanama PERPeru PHLPhilippines PNGPapuaNewGuinea POLPoland PRIPuertoRico PRKNorthKorea PRTPortugal PRYParaguay QATQatar ROURomania RUSRussia RWARwanda XXWWesternSahara SAUSaudiArabia SDNSudan SDSSouthSudan SENSenegal SLBSolomonIslands SLESierraLeone SLVElSalvador XXSSomaliland SOMSomalia SRBRepublicofSerbia SURSuriname SVKSlovakia SVNSlovenia SWESweden SWZSwaziland SYRSyria TCDChad TGOTogo THAThailand TJKTajikistan TKMTurkmenistan TLSEastTimor TTOTrinidadandTobago TUNTunisia TURTurkey TWNTaiwan TZAUnitedRepublicofTanzania UGAUganda UKRUkraine URYUruguay USAUnitedStatesofAmerica UZBUzbekistan VENVenezuela VNMVietnam VUTVanuatu PSEWestBank YEMYemen ZAFSouthAfrica ZMBZambia ZWEZimbabwe",z);
  for(i=1;i<=nz;++i){
    dnc[substr(z[i],1,3)]=i;
    dnc[substr(z[i],4)]=i;
    #dnn[substr(z[i],4)]=i;
  }
  ...
  npal=split("97 94 96 92 93 91 95",pal);
  dmin=dmax=0;
}

$2 ~ /-{0,1}[0-9]/ {
  if($1 in dnc)key=dnc[$1];else next;
  v=0+$2;
  data[key]=v;
  if(D)print key, data[key];
  if(v>dmax)dmax=v;
  if(v<dmin)dmin=v;
}

END {
  #print "dmin", dmin, "dmax", dmax; exit;
  for (t=0;t<N;++t) {
  #for (t=0;t<1;++t) {
    globe(t,w,nw);
    system("sleep 0.1");
